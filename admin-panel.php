    ...
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      $check_query = mysqli_query($con,"select apptime from appointmenttb where doctor='$doctor' and appdate='$appdate' and apptime='$apptime'");
        ...
        if(mysqli_num_rows($check_query)==0){
          ...
          if($query)
          {
            echo "<script>alert('Seu compromisso foi agendado com sucesso');</script>";
          }
          else{
...
function generate_bill(){
  ...
  while($row = mysqli_fetch_array($query)){
    $output .= '
    <label> ID Paciente : </label>'.$row["pid"].'<br/><br/>
    <label> ID do Compromisso : </label>'.$row["ID"].'<br/><br/>
    <label> Nome do paciente : </label>'.$row["fname"].' '.$row["lname"].'<br/><br/>
    <label> Nome do Médico : </label>'.$row["doctor"].'<br/><br/>
    <label> Data : </label>'.$row["appdate"].'<br/><br/>
    <label> Horário : </label>'.$row["apptime"].'<br/><br/>
    <label> Doença : </label>'.$row["disease"].'<br/><br/>
    <label> Alergias : </label>'.$row["allergy"].'<br/><br/>
...
if(isset($_GET["generate_bill"])){
  ...
  $content .= '
      <br/>
      <h2 align ="center"> Simplifica MED</h2></br>
...
.btn-primary{
  ...
  <div class="col-md-4" style="max-width:25%; margin-top: 3%">
    <div class="list-group" id="list-tab" role="tablist">
      ...
      <a class="list-group-item list-group-item-action" href="#list-pres" id="list-pres-list" role="tab" data-toggle="list" aria-controls="home">Prescrição</a>
...
<div class="tab-pane fade" id="app-hist" role="tabpanel" aria-labelledby="list-pat-list">
                      ...
                      echo "Cancelado pelo Medico!";
                        ...
                        { ?>
	                        ...
	                        <a href="admin-panel.php?ID=<?php echo $row['ID']?>&cancel=update" 
                              ...
                              title="Cancela o agendamento" tooltip-placement="top" tooltip="Remove"><button class="btn btn-danger">Cancelar</button></a>
	                        <?php } else {
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <!-- <a href="admin-panel.php?ID=" 
                              ...
                              <input type = "submit" onclick="alert('Fatura paga com sucesso');" name ="generate_bill" class = "btn btn-success" value="Pay Bill"/>
                              </a>
                              </td>
                              </form>
      ...
      <div class="tab-pane fade" id="list-settings" role="tabpanel" aria-labelledby="list-settings-list">
        <form class="form-group" method="post" action="func.php">
          ...
          <br>
          <input type="submit" name="doc_sub" value="Adcionar Médico" class="btn btn-primary">
        </form>
      </div>
